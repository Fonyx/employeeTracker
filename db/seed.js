...
let departmentQuery = `INSERT INTO department(name)
VALUES
('Marketing'),
('Engineering'),
('HR'),
...
let roleQuery = `INSERT INTO role(title, salary, department_id)
VALUES
-- marketing
...
let headQuery = `INSERT INTO employee(first_name, last_name, role_id) VALUES
('James', 'Smith', 1);`;

let employeesQuery = `INSERT INTO employee(first_name, last_name, role_id, manager_id) VALUES
('Hoth', 'Aier', 2, 1),
('Jane', 'Doe', 3, 1),
('Ellen', 'Ran', 4, 1),
('Mark', 'Dong', 5, 1),
...
async function main(){
    ...
    await conn.execute(departmentQuery);
    await conn.execute(roleQuery);
    await conn.execute(headQuery);
    await conn.execute(employeesQuery);
    console.log('Seeded');
}
